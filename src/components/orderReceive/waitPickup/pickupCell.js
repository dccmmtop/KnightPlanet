  ...
  async _getLists(orderId) {
    ...
    return(
      <View style={commonStyles.listItem} key={data.expressOrderId}>
        <TouchableOpacity onPress={() => this.props.navigation.navigate('OrderDetail', {type: 'waitpick', data: data})}>
          <Flex justify='between' style={commonStyles.line1}>
            <Flex>
              <Text style={[commonStyles.p1, {marginRight: 10}]}>取件倒计时</Text>
          ...
          <Modal isVisible={this.state.isModalVisible}>
            <Flex direction='column' style={{backgroundColor: '#fff'}}>
              ...
              <Flex justify='center' style={{marginBottom: 35}}>
                <Text style={commonStyles.contain_txt}>确定<Text style={[commonStyles.font7, {marginHorizontal: 5}]}>{data.fromBusinessName}</Text>的商品已取件？</Text>
              </Flex>
              <Flex justify='between' style={[{width: '100%'}, commonStyles.border_top]}>
                ...
                <TouchableOpacity style={{flex: 1}}  onPress={() =>this._comfirmPick(data.expressOrderId)}>
                  <Flex justify='center'>
