...
import ListItem from './deliveringCell'
...
export default class WaitReceive extends Component {
  ...
  componentDidMount() {
    DeviceEventEmitter.addListener('delivering', () => {
      this._onRefresh();
    })
  }
   // 获取订单列表数据
  async _requestData(isReload) {
    ...
    dataList = data.map((item) => {
      // 配送倒计时计算
      const deliveryTime = new Date(item.expectDeliveryTime);
      const nowTime = new Date();
      let restTime = (parseInt(deliveryTime - nowTime) / 1000 / 60).toFixed(0); //计算结果为分钟
  ...
  render() {
    const noData =  
      <Flex direction='column' justify='center' style={{height: this.state.fHeight}}>
        <Image
